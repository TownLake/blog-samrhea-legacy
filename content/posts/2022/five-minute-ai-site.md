...
Here is an example when I asked it to create a Monet style painting of a Texas Longhorn wearing a birthday party hat.
...
My description of this image as “entirely new” pushes some boundaries. This image has never existed before. OpenAI is not clipping and copy-pasting a photo of a a longhorn and a photo of a hat and a photo of a Monet landscape. The service looked at, basically, all images that exist on the Internet and knows how to create new ones now. Most people have a rough idea of what a Monet painting should look like, but OpenAI also consumed thousands of photos of Longhorn cattle taken by photographers and shared on the Internet. This model relies on that work but does not credit them; the attribution would be nearly impossible. I don’t imagine bovine photographers are terribly concerned by this, but that does pose a dilemma for artists. I’m not going to use this post to explore that topic, experts and the folks impacted have discussed it in much more eloquent depth, but I think the framing matters to understand how this works.
...
The thing writes emails and medical patient notes and short stories with the right prompt. I asked my dad, who is an attorney, to join a video call and we had GPT start writing hypothetical client emails about case law. For good reason, OpenAI has configured some bumpers with how you can use the tool - GPT is very nervous about impersonating a lawyer and tries hard not to give legal advice. Convincing it to do so takes some finagling; I can’t imagine what you can do without those guardrails.
...
* ChatGPT remembered that I asked it to create something editorial as part of the request for code, the Lisbon list, and the output is pretty vanilla but not that different from what I’d tell a friend.
* ChatGPT created a website that would provide that editorial on a webpage.
...
As an extension of that assembly line concept, I've heard some critiques which call out inaccuracies in ChatGPT output. Those occur. One dangerous aspect about the service is that it can both be wrong and still confident that it is right. However, finding flaws in the output is only a good use for better model training. Using those flaws to write it off feels like an artisan shoe cobbler pointing out that the new sneaker assembly line produced a shoe with a shoelace eyelet missing. Sure, that’s going to happen, but the volume of sneakers produced from a factor means more people can afford sneakers and over time the quality improves too.
...
Weirder things are about to happen. I joked with a friend the other day that, at some point, I'll have my first text conversation with what I think is a human I know but I'll be interacting with an AI the entire time - the movie *Her* feels possible now. I've jokingly challenged the Product Managers on my team that the first to write a Product Requirements Document (PRD) with ChatGPT that I approve wins lunch. I think one of them already did it. I promise ChatGPT did not write this blog post but I bet I'll try that someday. Just to see if I can. Buckle up because this is going to get rowdy and strange and either wonderful or terrifying or a mix of both.
